...
class ReportController extends Controller
{
    ...
    public function reportdashboard3(Request $request){
        $newArray = array();
        //Stock Keluar
        if(empty($request->id_product)){
            $id_product=0;
        ...
        $sqlstockkeluar = 'SELECT product_code, product_name, SUM(trans_qty) AS stock_keluar, trans_date, doctype_id FROM inventory_journal a
                INNER JOIN products b ON a.`id_product`=b.`id`
                WHERE trans_date between ? and  ? AND doctype_id=2 and (a.id_product=? or 0=?) and trans_loc = ?
                GROUP BY product_code, trans_date, doctype_id, product_name
                UNION ALL
                SELECT product_code, product_name, SUM(trans_qty) AS stock_keluar, trans_date, doctype_id FROM inventory_journal a
                INNER JOIN products b ON a.`id_product`=b.`id`
                WHERE trans_date between ? and  ? AND doctype_id=4 and (a.id_product=? or 0=?) and trans_loc = ?
                GROUP BY product_code, trans_date, doctype_id, product_name
                HAVING SUM(trans_qty)<0';
        $stockkeluar = DB::select($sqlstockkeluar,[$request->start_date,$request->end_date,$id_product,$id_product,$request->id_lokasi,$request->start_date,$request->end_date,$id_product,$id_product,$request->id_lokasi]);

        $newArray['stocKeluar'] = $stockkeluar;

        //Omset
        $sqlomset = 'SELECT SUM(grandtotal) AS omset, date_header FROM sales_invoice_h a
                WHERE flow_seq IN(100,120) AND date_header between ? and ? and location_id = ?
                GROUP BY date_header
                HAVING SUM(grandtotal)>0';
        $omset = DB::select($sqlomset,[$request->start_date, $request->end_date, $request->id_lokasi]);
